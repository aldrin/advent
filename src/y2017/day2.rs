...
pub fn part1(matrix: &[Vec<u32>]) -> u32 {
    matrix.iter().map(Vec::as_slice).map(helpers::range).sum()
}


// Find the only two numbers in each row where one evenly divides the other, divide them, and add up
// each line's result
pub fn part2(matrix: &mut [Vec<u32>]) -> u32 {
    matrix.iter_mut().map(helpers::multiples).sum()
}
...
pub mod helpers {
    ...
    pub fn read_matrix(input: &str) -> Vec<Vec<u32>> {
        ...
        input
            ...
            .map(|l| l.split_whitespace().filter_map(|s| u32::from_str(s).ok()).collect())
            .collect()
    }
...
fn examples() {
    ...
    3 8 6 5
    ";
    assert_eq!(part1(&helpers::read_matrix(one)), 18);
    assert_eq!(part2(&mut helpers::read_matrix(two)), 9);
}

#[test]
fn solution() {
    let mut digits = helpers::read_matrix(include_str!("input/2"));
